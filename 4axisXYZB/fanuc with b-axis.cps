...
description = "FANUC - Inverse Time and B-axis";
vendor = "Fanuc";
...
longDescription = "Generic Fanuc post illustrating inverse time feed with an B-axis.";
...
//makeAAxisOtherWay: false, // make the A-axis rotate the opposite way //change to B Axis
...
//makeAAxisOtherWay: {title:"Reverse A-axis", description:"Makes the A-axis rotate the opposite way.", type:"boolean"}, //change to B Axis
...
function onOpen() {
  ...
  if (true) {
    //var aAxis = createAxis({coordinate:0, table:true, axis:[(properties.makeAAxisOtherWay ? -1 : 1) * -1, 0, 0], cyclic:true, preference:1}); //change to B Axis
    ...
    optimizeMachineAngles2(1); // map tip mode 0=a, 1=b, 2=c
  }
...
/*
  if (programName) {
    ...
    if (programComment) {
      //writeln("O" + oFormat.format(programId) + " (" + filterText(String(programComment).toUpperCase(), permittedCommentChars) + ")"); //LinuxCNC
    } else {
      //writeln("O" + oFormat.format(programId)); //LinuxCNC
    }
...
function writeRetract() {
  ...
  for (var i = 0; i < arguments.length; ++i) {
    ...
    case X:
      ...
      words.push("X" + xyzFormat.format(machineConfiguration.hasHomePositionX() ? machineConfiguration.getHomePositionX() : 10));
      break;
    case Y:
      //words.push("Y" + xyzFormat.format(machineConfiguration.hasHomePositionY() ? machineConfiguration.getHomePositionY() : 0));
      words.push("Y" + xyzFormat.format(machineConfiguration.hasHomePositionY() ? machineConfiguration.getHomePositionY() : 350));
      break;
    case Z:
